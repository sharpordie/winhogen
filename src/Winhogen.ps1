...
Function Update-Antidote {
    ...
    If (-Not $Updated) {
        ...
        write-output msiexec.exe /i "$Using:Modules\Antidote11.msi" $Using:Adjunct /qn
        ...
        # Foreach ($MspFile In $(Get-Item "$RootDir\Updates\*.msp")) { Invoke-Gsudo { & msiexec.exe /p "$Using:MspFile.FullName" /qn } }
