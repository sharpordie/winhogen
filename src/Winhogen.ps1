...
Function Invoke-Fetcher {
    ...
    Switch ($Fetcher) {
        ...
        "Filecr" {
            ...
            $Browser = $Handler.Chromium.LaunchAsync(@{ "Headless" = $False }).GetAwaiter().GetResult()
            $WebPage = $Browser.NewPageAsync().GetAwaiter().GetResult()
            $WebPage.SetViewportSizeAsync(1400, 1700).GetAwaiter().GetResult()
...
Function Update-Antidote {
    ...
    If (-Not $Updated) {
        ...
        Invoke-Gsudo { echo "$Using:Modules\Antidote11.msi" $Using:Adjunct }
        # Invoke-Gsudo { & msiexec.exe /i /qn "$Using:Modules\Antidote11.msi" $Using:Adjunct }
        # $Adjunct = "TRANSFORMS=`"$Modules\Antidote11-Module-francais-Interface-en.mst`""
        # Invoke-Gsudo { & msiexec.exe /i /qn "$Using:Modules\Antidote11-Module-francais.msi" $Using:Adjunct }
        # $Adjunct = "TRANSFORMS=`"$Modules\Antidote11-English-module-Interface-en.mst`""
        # Invoke-Gsudo { & msiexec.exe /i /qn "$Using:Modules\Antidote11-English-module.msi" $Using:Adjunct }
        # $Adjunct = "TRANSFORMS=`"$Modules\Antidote-Connectix11-Interface-en.mst`""
        # Invoke-Gsudo { & msiexec.exe /i /qn "$Using:Modules\Antidote-Connectix11.msi" $Using:Adjunct }
        # Foreach ($MspFile In $(Get-Item "$RootDir\Updates\*.msp")) { Invoke-Gsudo { & msiexec.exe /p /qn "$Using:MspFile.FullName" } }
